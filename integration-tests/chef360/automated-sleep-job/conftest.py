...
def pytest_addoption(parser):
    ...
    parser.addoption("--NODE_USER", action="store", default="ubuntu", help="Value for NODE_USER")
...
def get_env_variables(request):
    ...
    env4 = request.config.getoption("--NODE_USER")
    env5 = request.config.getoption("--NODE_PORT")
    return {"PEM_KEY": env1, "NODE_FQDN": env2, "LICENSE_ID": env3, "NODE_USER": env4, "NODE_PORT": env5}
